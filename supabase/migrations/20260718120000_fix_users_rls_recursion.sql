...
-- Root cause: users RLS policies used a subquery that re-read public.users
-- (EXISTS (SELECT 1 FROM public.users u WHERE u.id = auth.uid() AND u.role='admin')),
-- so any query on users re-evaluated the policy -> infinite loop.
-- Fix: move the admin check into a SECURITY DEFINER function that runs outside
-- RLS, and reference it from the policies instead of self-referencing.
